...
#
# This script:
#   - Creates Astonia.app in ./distrib
#   - Copies bin/ and res/
#   - Fixes dylib dependencies into Resources/bin
#   - Generates AppIcon.icns from res/moa3.ico
#
# It does *NOT* perform code signing. Use sign_macos_app.sh for that.
...
# Script is expected at build/tools/package_macos.sh → project root is two levels up
...
fix_deps() {
  ...
  otool -L "$bin" | tail -n +2 | awk '{print $1}' | while read -r dep; do
    ...
    # Skip @rpath/@loader_path already-relative entries
    case "$dep" in
      @*) continue ;;
    esac
    ...
    if [[ ! -f "$dest" ]]; then
      ...
      # Set the install_name id of the copied dylib
    ...
    # Rewrite this binary's reference to the dependency
...
make_icns_from_ico() {
  local ICO_IN="$1"           # e.g. res/moa3.ico
  ...
  # If [0] ever gives you a tiny icon, change to [1] or [2].
  ...
  # Helper to resize into the iconset
  resize_icon() {
...
if [[ -f "$APP_RES_RES/moa3.ico" ]]; then
  make_icns_from_ico "$APP_RES_RES/moa3.ico" "$APP_RESOURCES/AppIcon.icns"
  rm "$APP_RES_RES/moa3.ico"
else
  echo "WARNING: $APP_RES_RES/moa3.ico not found; AppIcon.icns will not be created." >&2
fi

# Strip any *.dSYM directories if present (just in case)
find "$APP_RES_BIN" -name "*.dSYM" -exec rm -rf {} + 2>/dev/null || true

# ---------------------------------------------------------------------------
# Launcher script (Contents/MacOS/astonia)
# ---------------------------------------------------------------------------

cat > "$LAUNCHER" <<'EOF'
#!/bin/bash
# Launcher for Astonia .app
# Keeps the existing bin/res layout expected by moac.

set -e

# Resolve App bundle structure:
#   MacOS/astonia           (this script)
#   Resources/bin/moac      (binary)
#   Resources/res/...       (assets)
APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
RESOURCES="$APP_DIR/Resources"

# Optional: set working dir to Resources so logs, etc. land near assets
cd "$RESOURCES" || exit 1

# Exec the actual game binary, forwarding all args
exec "$RESOURCES/bin/moac" "$@"
EOF
...
echo "==> macOS .app bundle created (UNSIGNED):"
echo "    $APP_BUNDLE"
...
static void exec_cmd(int cmd,int a) {
    ...
    switch (cmd) {
        ...
        case CMD_INV_LOOK:
            // Try to show cached description first (old client mode optimization)
            if (!hover_show_cached_desc(invsel)) {
                cmd_look_inv(invsel);
            }
        ...
        case CMD_WEA_LOOK:
            // Try to show cached description first (old client mode optimization)
            if (!hover_show_cached_desc(weatab[weasel])) {
        ...
        case CMD_CON_LOOK:
            // Try to show cached description first (old client mode optimization)
            if (!hover_show_cached_desc(INVENTORYSIZE+consel)) {
                cmd_look_con(consel);
            }

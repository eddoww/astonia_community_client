...
int png_load_helper(struct png_helper *p)
{
	...
	if (png_get_channels(p->png_ptr, p->info_ptr) != p->bpp / 8) {
		fclose(fp);
		png_destroy_read_struct(&p->png_ptr, &p->info_ptr, (png_infopp)NULL);
		fprintf(stderr, "channels!=format\n");
		return -1;
	}
...
int convert_to_wall(struct sdl_image *si, int sprite, int sdl_scale, struct png_helper *p)
{
	...
	write_png_file(
	    fname, si->pixel, si->xres * sdl_scale, si->yres * sdl_scale, (-si->yoff * 2 - si->yres) * sdl_scale);
#endif
...
void write_png_file(char *filename, uint32_t *pixel, int width, int height, int heightfill)
{
	...
	png_set_IHDR(png, info, width, height + heightfill, 8, PNG_COLOR_TYPE_RGB_ALPHA, PNG_INTERLACE_NONE,

...
_Static_assert(sizeof(gpu_sprite_instance_t) == 128, "gpu_sprite_instance_t must be 128 bytes to match GLSL struct");
...
static SDL_GPUShader *load_shader(const char *filename, SDL_GPUShaderStage stage, Uint32 num_samplers,
    Uint32 num_storage_buffers, Uint32 num_uniform_buffers)
{
	...
	if (!shader) {
		note("gpu_batch load_shader: SDL_CreateGPUShader failed for %s: %s", filename, SDL_GetError());
	}
...
static bool create_quad_vbo(void)
{
	...
	SDL_GPUBufferRegion dst = {.buffer = batch_state.quad_vbo, .offset = 0, .size = sizeof(quad_vertices)};
...
static bool create_instance_buffers(void)
{
	...
	SDL_GPUBufferCreateInfo buf_info = {
	    .usage = SDL_GPU_BUFFERUSAGE_GRAPHICS_STORAGE_READ, .size = (Uint32)buffer_size};
	...
	// Transfer buffer
	SDL_GPUTransferBufferCreateInfo transfer_info = {
	    .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD, .size = (Uint32)buffer_size};
	...
	// CPU staging buffer
	batch_state.instances =
	    (gpu_sprite_instance_t *)calloc((size_t)GPU_BATCH_MAX_SPRITES, sizeof(gpu_sprite_instance_t));
...
{
	SDL_GPUSamplerCreateInfo info = {.min_filter = SDL_GPU_FILTER_NEAREST, // Pixel art - no filtering
...
static bool create_pipeline(void)
{
	...
	SDL_GPUVertexAttribute vertex_attrs[2] = {{.location = 0,
	                                              .buffer_slot = 0,
	...
	SDL_GPUVertexInputState vertex_input = {.vertex_buffer_descriptions = &vb_desc,
	...
	SDL_GPUGraphicsPipelineTargetInfo target_info = {.color_target_descriptions = &color_desc,
	    .num_color_targets = 1,
	    .depth_stencil_format = SDL_GPU_TEXTUREFORMAT_INVALID,
	    .has_depth_stencil_target = false};

	SDL_GPUGraphicsPipelineCreateInfo pipeline_info = {.vertex_shader = batch_state.vertex_shader,
	    ...
	    .multisample_state = {.sample_count = SDL_GPU_SAMPLECOUNT_1, .sample_mask = 0xFFFFFFFF, .enable_mask = false},
	...
	if (!batch_state.pipeline) {
		note("gpu_batch create_pipeline: SDL_CreateGPUGraphicsPipeline failed: %s", SDL_GetError());
		return false;
	}
...
SDL_GPURenderPass *gpu_batch_begin_frame(SDL_GPUCommandBuffer *cmd, SDL_GPUTexture *target)
{
	...
	// Begin render pass
	SDL_GPUColorTargetInfo color_target = {.texture = target,
	    .mip_level = 0,
...
void gpu_batch_flush(void)
{
	...
	SDL_GPUTransferBufferLocation src = {.transfer_buffer = batch_state.instance_transfer, .offset = 0};
	SDL_GPUBufferRegion dst = {.buffer = batch_state.instance_buffer, .offset = 0, .size = (Uint32)data_size};
	SDL_UploadToGPUBuffer(copy_pass, &src, &dst, false);
	SDL_EndGPUCopyPass(copy_pass);
	...
	// Bind vertex buffer
	SDL_GPUBufferBinding vb_binding = {.buffer = batch_state.quad_vbo, .offset = 0};
	...
	SDL_GPUTextureSamplerBinding tex_binding = {.texture = batch_state.current_texture, .sampler = batch_state.sampler};
	SDL_BindGPUFragmentSamplers(current_pass, 0, &tex_binding, 1);

	// Push frame uniforms (screen size)
	float frame_uniforms[4] = {batch_state.screen_width, batch_state.screen_height, 1.0f / batch_state.screen_width,
...
void gpu_batch_add_sprite(
    SDL_GPUTexture *texture, const SDL_FRect *dest, const SDL_FRect *src, const gpu_sprite_instance_t *instance)
{
	if (!batch_state.in_batch) {
		return;
	}
...
void gpu_batch_add_sprite_simple(SDL_GPUTexture *texture, const SDL_FRect *dest, const SDL_FRect *src, float alpha)
{
...
void gpu_batch_dump(FILE *fp)
{
	...
	fprintf(fp, "Screen: %.0fx%.0f\n", (double)batch_state.screen_width, (double)batch_state.screen_height);
	fprintf(fp, "\nLast frame:\n");

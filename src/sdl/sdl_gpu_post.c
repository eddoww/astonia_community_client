...
static bool create_quad_vbo(void)
{
	...
	SDL_GPUBufferRegion dst = {.buffer = postfx_state.quad_vbo, .offset = 0, .size = sizeof(quad_vertices)};
...
{
	SDL_GPUSamplerCreateInfo info = {.min_filter = SDL_GPU_FILTER_LINEAR,
...
static bool create_pipeline(void)
{
	...
	SDL_GPUColorTargetDescription color_desc = {.format = SDL_GPU_TEXTUREFORMAT_B8G8R8A8_UNORM, // Swapchain format
	...
	SDL_GPUGraphicsPipelineCreateInfo pipeline_info = {.vertex_shader = postfx_state.vertex_shader,
...
SDL_GPURenderPass *gpu_postfx_begin_scene(SDL_GPUCommandBuffer *cmd)
{
	...
	SDL_GPUColorTargetInfo color_target = {.texture = postfx_state.scene_texture,
	    .mip_level = 0,
...
void gpu_postfx_present(SDL_GPUCommandBuffer *cmd, SDL_GPUTexture *swapchain_texture)
{
	...
	// Bind vertex buffer
	SDL_GPUBufferBinding vb_binding = {.buffer = postfx_state.quad_vbo, .offset = 0};
	SDL_BindGPUVertexBuffers(pass, 0, &vb_binding, 1);

	// Bind scene texture and sampler
	SDL_GPUTextureSamplerBinding tex_binding = {.texture = postfx_state.scene_texture, .sampler = postfx_state.sampler};
...
void gpu_postfx_dump(FILE *fp)
{
	...
	fprintf(fp, "Scene texture: %p (%dx%d)\n", (void *)postfx_state.scene_texture, postfx_state.scene_width,
	...
	fprintf(fp, "  Tint: rgb=(%.2f,%.2f,%.2f), intensity=%.2f\n", (double)postfx_state.params.tint_r,
	    (double)postfx_state.params.tint_g, (double)postfx_state.params.tint_b,

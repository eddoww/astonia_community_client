	...
	if (!preload || preload == 3) {
		...
		if (st->xres > 0 && st->yres > 0) {
			...
			if (use_gpu_rendering) {
				if (st->pixel) {
					...
					if (st->gpu_tex) {
						...
						if (gpu_tex_fail_count++ < 10) {
							note("sdl_make: gpu_texture_create failed for sprite %d (%dx%d)", st->sprite, st->xres,
							    st->yres);
						}
					}
				} else {
					...
					if (no_pixel_count++ < 10) {
						note("sdl_make: no pixel data for GPU texture sprite %d - flags=%d preload=%d", st->sprite,
						    flags_load(st), preload);
					}
					st->gpu_tex = NULL;
				}
				...
				texture = SDL_CreateTexture(sdlren, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STATIC,
				    st->xres * sdl_scale, st->yres * sdl_scale);
				if (!texture) {
					warn("SDL_texture Error: %s in sprite %d (%s, %d,%d) preload=%d", SDL_GetError(), st->sprite,
					    st->text, st->xres, st->yres, preload);
					return;
				}
